...
def place_order(request):
    if request.method == "POST":
        ...
        order_total = request.data['total']
        user_id = request.data['userId']
        print(user_id, order_total),
        ...
        cart_items = [CartItem(
            product=item.product, quantity=item.quantity, cart=item.cart) for item in list_items]
        for cart_item in cart_items:
            order_product = OrderProduct()
            slug = cart_item.product.slug
            stocked_product = get_object_or_404(
